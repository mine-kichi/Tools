...
int imReadBmpHeader( char *strFileName, FILE **fpp,
					...
					BITMAPINFOHEADER* pbih){
						...
						errno_t error;
						if( (error=fopen_s(&*fpp,strFileName,"rb")) != 0 ){
							sprintf_s( strErrMsg, "imBmpReadHeader:File Open Error: %s\n", strFileName );
						...
						if       ( bxSize == 40 ){ // Windows bitmap
							...
							sprintf_s( strErrMsg, "imBmpReadHeader:File Open Error (Imcompatible BMP): %s\n", strFileName );
							imErrMessageHandle(strErrMsg);
							fclose(*fpp);
							return -1;
						}else{
							sprintf_s( strErrMsg, "imBmpReadHeader:File Open Error (Bloaken File?): %s(bxSize=%ld)\n", strFileName,bxSize );
							imErrMessageHandle(strErrMsg);
							fclose(*fpp);
							return -1;
						}
						...
						if( pbih->biHeight * iVerticalStep != pbih->biSizeImage ){
							sprintf_s( strErrMsg, "imBmpReadHeader:File Open Error (Illigal file size): %s\n", strFileName );
							imErrMessageHandle(strErrMsg);
							fclose(*fpp);
							return -1;
						}
...
int imBmpReadBody( IM_RGB* pimRGB, FILE *fp, char* strFileName, 
				  ...
				  BITMAPINFOHEADER* pbih){
					  ...
					  if( (pBmpData=(unsigned char*)malloc( pbih->biSizeImage ))==NULL ){
						  sprintf_s( strErrMsg, "imBmpReadBody:Memory Allocation Error: %s\n", strFileName );
						  imErrMessageHandle(strErrMsg);
						  fclose(fp);
						  return -1;
					  }
					  ...
					  if( ret != pbih->biSizeImage ){
						  sprintf_s( strErrMsg, "imBmpReadBody:Readable data is not enough: %s\n", strFileName );
						  imErrMessageHandle(strErrMsg);
						  fclose(fp);
						  return -1;
					  }
...
int imBmpReadAgain( IM_RGB* pimDST, char *strFileName ){
	...
	//Pointer Check
	if( pimDST==NULL ){
		sprintf_s( strErrMsg, "imBmpReadAgain:pimDST is NULL pointer\n" );
		imErrMessageHandle(strErrMsg);
		return -1;
	}
	...
	if( (pimDST->iSizeX!=bih.biWidth) || (pimDST->iSizeY!=bih.biHeight) ){
		sprintf_s( strErrMsg, "imBmpReadAgain:Incompatible size: %s\n", strFileName );
		imErrMessageHandle(strErrMsg);
		fclose(fp);
		return -1;
	}
...
IM_RGB* imBmpRead2( char *strFileName ){
	...
	if( (pimRGB=imMalloc(bih.biWidth,bih.biHeight)) ==NULL){
		sprintf_s( strErrMsg, "imBmpRead:Memory Allocation Error: %s\n", strFileName );
		imErrMessageHandle(strErrMsg);
		fclose(fp);
		return NULL;
	}
...
IM_RGB* imBmpRead( char *strFileName ){
	...
	if( (error=fopen_s(&fp,strFileName,"rb"))!=0 ){
		sprintf_s( strErrMsg, "imBmpRead:File Open Error: %s\n", strFileName );
		imErrMessageHandle(strErrMsg);
		return pimRGB;
	}
	/* Read Bitmap HEADER -------------------------------------------*/
	...
	if       ( bxSize == 40 ){ // Windows bitmap
		...
		sprintf_s( strErrMsg, "imBmpRead:File Open Error (Imcompatible BMP): %s\n", strFileName );
		imErrMessageHandle(strErrMsg);
		return pimRGB;
	}else{
		sprintf_s( strErrMsg, "imBmpRead:File Open Error (Bloaken File?): %s(bxSize=%ld)\n", strFileName,bxSize );
		imErrMessageHandle(strErrMsg);
		return pimRGB;
	}
	/* Jadge Readable -------------------------------------------------*/
	...
	if( bih.biHeight * iVerticalStep != bih.biSizeImage ){
		sprintf_s( strErrMsg, "imBmpRead:File Open Error (Illigal file size): %s\n", strFileName );
		imErrMessageHandle(strErrMsg);
		return pimRGB;
	}
	/* Memory Allocation   -------------------------------------------*/
...
int imBmpWrite( IM_RGB* pimRGB, char* strFileName ){
	...
	if( (pBmpData=(unsigned char*)malloc( bih.biSizeImage ))==NULL ){
		sprintf_s( strErrMsg, "imBmpWrite:Memory Allocation Error: %s\n", strFileName );
		imErrMessageHandle(strErrMsg);
		return ret;
	}
	...
	if( (error=fopen_s(&fp,strFileName,"wb"))!=0 ){
		sprintf_s( strErrMsg, "imBmpWrite:File Open Error: %s\n", strFileName );
		imErrMessageHandle(strErrMsg);
		return ret;
	}
...
int  imHalfSynth(IM_RGB* pimDst, IM_RGB* pimLeft,IM_RGB* pimRight){
	...
	if( (!imIsSameSize(pimLeft, pimRight))||
		(!imIsSameSize(pimLeft, pimDst  )) ){
			sprintf_s( strErrMsg, "imHalfSynth:Imcompatible size:\n\
								Left :%d x %d\n\
								Right:%d x %d\n\
								Dst  :%d x %d\n",
...
void CSVwrite(IM_RGB *pimRGB, char *strFileName){
	...
	if(sizeof(ImImage)==sizeof(unsigned int)){
		...
		strcat_s(fname1,strFileName);
		strcat_s(fname2,strFileName);
		strcat_s(fname3,strFileName);
		char strErrMsg[IM_BMP_BUF_SIZE];
		
		error =fopen_s(&csvfp1,fname1,"w");
		error =fopen_s(&csvfp1, fname2,"w");
		error =fopen_s(&csvfp1, fname3,"w");
		...
		if( csvfp1 == NULL || csvfp2 == NULL || csvfp3 == NULL){
			sprintf_s( strErrMsg, "CSVwrite:Memory Allocation Error: %s or %s or %s\n", fname1,fname2,fname3 );
			imErrMessageHandle(strErrMsg);
			return;
		}
...
IM_RGB* imPPMread( char *strFileName ){
	...
	if( (error=fopen_s(&fp,strFileName,"r"))!=0 ){
		sprintf_s( strErrMsg, "imPPMread:File Open Error: %s\n", strFileName );
		imErrMessageHandle(strErrMsg);
		return pimRGB;
	}
	/* Read PPM HEADER -------------------------------------------*/
	//fgets(format,5,fp);
	fgets(temp, 75, fp) ;
	sscanf_s(temp,"%s",format);//�t�@�C���`���̏��̓ǂݍ���

	if (format[0]!='P'){
		sprintf_s( strErrMsg, "imPPMread:File type isn't PPM: %s\n", strFileName );
		imErrMessageHandle(strErrMsg);
		return pimRGB;
	}
	sscanf_s(format,"P%d",&type);
	if(type != 3 && type != 6){
		sprintf_s( strErrMsg, "imPPMread:File type isn't PPM: %s\n", strFileName );
		imErrMessageHandle(strErrMsg);
		return pimRGB;
	...
	sscanf_s(comment,"%d%d",&X_Size,&Y_Size);//�摜�T�C�Y�̓ǂݍ���

	fgets(temp, 75, fp);
	sscanf_s(temp,"%d",&lm);

	if (X_Size<=0||Y_Size<=0||lm<=0){
		sprintf_s( strErrMsg, "imPPMread:PPM range is fault: %s\n", strFileName );
		imErrMessageHandle(strErrMsg);
		return pimRGB;
	}
	...
	for (idx=0;idx<(X_Size*Y_Size);idx++){
		fgets(temp, 75, fp); 
		sscanf_s(temp,"%d %d %d", &valR, &valG, &valB);
		pimRGB->R[idx]=valR;
		pimRGB->G[idx]=valG;
		pimRGB->B[idx]=valB;
	}
...
int imPPMwrite( IM_RGB* pimRGB, char* strFileNameorg, char* strFileNamedst){
	...
	if( (error=fopen_s(&ffp,strFileNameorg,"r"))!=0 ){
		sprintf_s( strErrMsg, "imPPMread:File Open Error: %s\n", strFileNameorg );
		imErrMessageHandle(strErrMsg);
		return -1;
	}
	...
	error=fopen_s(&ppm,strFileNamedst,"w");
	...
	if( ppm == NULL ){
		//        sprintf_s( strErrMsg, "CSVwrite:Memory Allocation Error: %s\n", ppm );
		sprintf_s( strErrMsg, "CSVwrite:Memory Allocation Error: %s\n", strFileNamedst );
		imErrMessageHandle(strErrMsg);
		return -1;
	}
...
int imPPMconvert( IM_RGB* pimRGB, char* strOutputFileName ){
	...
	error=fopen_s(&ppm,strOutputFileName,"w");
